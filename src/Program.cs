...
namespace markdown_composer
{
    public static class Program
    {
        ...
        {
            using var fs = new FileStream(DefaultWritePath, FileMode.OpenOrCreate, FileAccess.Write);
            using var sw = new StreamWriter(fs);
            sw.Write(
                new CompositionBuilder(rootFolder, makeToc)
                .FromReferenceFile(ReferenceFileName)
                .Composition.MarkdownText
            );
        }
